...
public class PersonalHistroyListActivity extends AppCompatActivity {
    ...
    private void setAdapter() {
        adapter = new PersonalHistoryAdapter(foods,user, R.layout.trends_listview_item, PersonalHistroyListActivity.this,stateCheckedMap);
    ...
    private void deleteData(List<Food> mCheckedData) {
        ...
        if (deleteFood.size()==1){
            Log.e("shanchu",Constant.URL_DELETE_HISTORY + user.getId()+"/"+deleteFood.get(0));
            request = new Request.Builder().url(Constant.URL_DELETE_HISTORY + user.getId()+"/"+deleteFood.get(0)).build();
        }else{
            request = new Request.Builder().url(Constant.URL_DELETE_HISTORYS + user.getId()+"/"+new Gson().toJson(deleteFood)).build();
        }
    ...
    private void requestData() {
        Request request = new Request.Builder().url(Constant.URL_HISTORY + user.getId()).build();
        final Call call = okHttpClient.newCall(request);
        //发送请求
        call.enqueue(new Callback() {
            @Override
